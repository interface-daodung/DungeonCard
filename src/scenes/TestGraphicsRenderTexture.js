...
export default class TestGraphicsRenderTexture extends Phaser.Scene {
    ...
    create() {
        ...
        backButton.on('pointerover', () => {
            backButton.setStyle({ fill: '#cccccc' });
        });
    ...
    createNavigationButtons(width, height) {
        ...
        prevButton.on('pointerover', () => {
            prevButton.setStyle({ fill: '#cccccc' });
        });
        
        prevButton.on('pointerout', () => {
            prevButton.setStyle({ fill: '#ffffff' });
        });
        ...
        nextButton.on('pointerover', () => {
            nextButton.setStyle({ fill: '#cccccc' });
        });
    ...
    createAnimationSprite(width, height) {
        ...
        if (!this.anims.exists(currentAnim)) {
            ...
            for (let i = 0; i < frameTotal; i++) {
                ...
                if (frame && frame.width > 0 && frame.height > 0) {
                    ...
                    try {
                        const canvas = frame.source.image;
