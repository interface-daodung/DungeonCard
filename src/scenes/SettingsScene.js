...
export default class SettingsScene extends Phaser.Scene {
    ...
    create() {
        ...
        if (window.gameEvents) {
            window.gameEvents.on('themeChanged', this.updateTheme, this);
            window.gameEvents.on('languageChanged', this.updateLanguage, this);
        }
    }
    ...
    createTopBackButton(width, height) {
        ...
        backButton.on('pointerover', () => {
            ...
            backButton.setTint(themeManager.getColor('buttonHover'));
        });
    ...
    createLanguagePanel(width, height) {
        const panelY = height * 0.2;
        
        // Language label với background đẹp
        const labelBg = this.add.graphics();
        labelBg.fillStyle(themeManager.getColor('cardBackground'));
        labelBg.fillRoundedRect(width * 0.1, panelY - 25, width * 0.8, 50, 10);
        labelBg.lineStyle(2, themeManager.getColor('cardBorder'));
    ...
    updateLanguage() {
        ...
        // Update back button
        this.children.list.forEach(child => {
            if (child.buttonText && child.buttonText.text === localizationManager.t('back')) {
                child.buttonText.setText(localizationManager.t('back'));
            }
        });
    }
    ...
    updateLanguageButtons() {
        this.languageButtons.forEach(({ button, lang }) => {
            ...
            if (isActive) {
                button.setTint(themeManager.getColor('buttonPrimary'));
            } else {
                button.clearTint();
            }
        });
    ...
    destroy() {
        ...
        if (window.gameEvents) {
            window.gameEvents.off('themeChanged', this.updateTheme, this);
            window.gameEvents.off('languageChanged', this.updateLanguage, this);
        }
        super.destroy();
    }
}
